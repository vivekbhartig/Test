...
function App() {
  ...
  const [setshowFinished, showFinished] = useState(true)
     ...
     {/* <h1 className='font-bold text-center text-3xl'>iTask - Manage your todos at one place</h1> */}
      {/* <div className=" bg-blue-300 my-5 rounded-lg p-5 "></div> */}
        <div className="addTodo my-5">
          ...
          <button onClick={handleAdd} disabled={todo.length<=3}className='bg-green-300 hover:bg-amber-200 p-2 py-1 
          text-sm font-bold text-white rounded-lg mx-6'>Save</button>
        </div>
        <input onchange={toggleFinished} type="checkbox" checked={showFinished} /> Show Finished
        <h2 className='text-lg font-extralight '>My ToDos</h2>
        <div className="todos">
          {todos.length ===0 && <div className= 'm-5 font-mono underline hover:decoration-dashed'> No Todos to display!</div> }
          {todos.map(item=>{
            
          return (showFinished || !item.isCompleted) && <div key={item.id} className={"todos flex w-1/2 my-3 justify-between"}>
            <input name={item.id}onChange={handleCheckbox} type="checkbox" checked=
            {todo.isCompleted} id="" />
